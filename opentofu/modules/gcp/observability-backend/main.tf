...
resource "google_logging_project_sink" "source" {
  ...
  project                = each.value
  name                   = "mindclade-central-observability"
...
resource "google_project_iam_member" "sink_writer" {
  ...
  lifecycle {
    precondition {
      condition     = each.value == google_logging_project_sink.source[each.key].writer_identity
      error_message = "Each approved sink writer must exactly match the provider-issued identity for its source project."
    }
  }
}
