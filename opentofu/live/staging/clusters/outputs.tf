...
output "region_authority" {
  ...
  precondition {
    condition = var.enabled ? (
      var.config.region == local.region_profile.primaryLocation &&
      alltrue([for zone in var.config.node_locations : startswith(zone, "${local.region_profile.primaryLocation}-")])
    ) : true
