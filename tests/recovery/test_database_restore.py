...
class DatabaseRestoreContractTest(unittest.TestCase):
    def test_database_module_preserves_recovery_controls(self):
        source = (ROOT / "opentofu/modules/gcp/cloud-sql-postgres/main.tf").read_text(encoding="utf-8")
        for control in (
    ...
    def test_runbook_requires_restore_isolation_and_evidence(self):
        runbook = (ROOT / "runbooks/database-failover-and-restore.md").read_text(encoding="utf-8").lower()
